...
Remarks: An instance of this class can be constructed to return any 3D geometric element that intersects the ray created by the origin and direction, or to return a subset of elements based on filtering and flags. The caller can opt to filter the results using an ElementFilter, or by applying a specific list of acceptable elements. The caller can also specify the type of object to be returned, which might be whole elements, geometry objects, or a combination. In all cases the caller is required to supply a 3D view for evaluation; the view and visibility settings on the input view will determine if a particular element is returned (for example, hidden elements will never be returned by this tool, nor will elements whose geometry is outside the section box of the view).The class is configured so that a single instance can be constructed and used for multiple evaluations of different rays. The results of the evaluation are not preserved between invocations on the same ReferenceIntersector. The class also offers an option to return element results encountered in Revit Links. When the flag is set, the results may include elements in the host document and in any RevitLinkInstance encountered, depending on the other flags set. See the remarks for for details on how the flags affect the results obtained from links.
Implements: IDisposable
...
Remarks: The ReferenceWithContext is used as the returned value from the method , ReferenceIntersector.Find(XYZ, XYZ), or ReferenceIntersector.FindNearest(XYZ, XYZ). It includes a reference intersecting a line extended in a certain direction from an origin point and the context of the geometric object, as the transform and proximity.
Implements: IDisposable
...
Description: Represents bounds of the array of repeating references in 0, 1, or 2 dimensions. (See ).
Implements: IDisposable
...
Implements: IDisposable
  ...
  PROPERTIES:
    LinkVisibility ColorFill { get; set; }
      Description: The color fill of RevitLinkGraphicsSettings. The color fill can be set to or . Only a view that supports color fill can have this property set to . To check if the view supports the color fill use to see if there is any supported color fill category for the view. The parameter must have a value assigned if this property is set to .
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
    ElementId LinkedViewId { get; set; }
      Description: The id of the linked view associated with this RevitLinkGraphicsSettings, or if no view is selected. If the is set to , then there must be a valid LinkedViewId.
    LinkVisibility LinkVisibilityType { get; set; }
      Description: The visibility type of RevitLinkGraphicsSettings. If the type is set to or , then the dependent properties of RevitLinkGraphicsSettings will be reset to their default state. The state of dependent properties can be changed later.
    LinkVisibility NestedLinks { get; set; }
      Description: The nested links of RevitLinkGraphicsSettings. The nested links can be set to or .
    LinkVisibility ObjectStyles { get; set; }
      Description: The object styles of RevitLinkGraphicsSettings. The object styles can be set to or .
    LinkVisibility ViewFilterType { get; set; }
      Description: The view filter type of RevitLinkGraphicsSettings. If the is set to , then the view filter type should be set to . If the is set to , then the view filter type should be set to . If the is set to , then the view filter type can be set to any value from enum. The parameter must have a value assigned if this property is set to . The value corresponds to None option in the UI.
    LinkVisibility ViewRange { get; set; }
      Description: The view range of RevitLinkGraphicsSettings. The view range can be set to or . Only a view that supports view range can have this property set to . To check if the view supports the view range use . The parameter must have a value assigned if this property is set to .
  ...
  METHODS:
    ...
    ElementId GetPhaseFilterId()
      Description: The linked document phase filter id of RevitLinkGraphicsSettings, or if there is no phase filter explicitly assigned.
    LinkVisibility GetPhaseFilterType()
      Description: The phase filter type of RevitLinkGraphicsSettings.
    ElementId GetPhaseId()
      Description: The linked document phase id of RevitLinkGraphicsSettings, or if there is no phase explicitly assigned.
    ...
    static bool IsViewRangeSupported(View view)
      Description: Returns true if the input view supports ViewRange settings of , false otherwise.
    void SetDiscipline(LinkVisibility disciplineType, ViewDiscipline discipline)
      ...
      @discipline: The discipline. If disciplineType is set to or the discipline must be set to .
    void SetPhase(LinkVisibility phaseType, ElementId phaseId)
      ...
      @phaseId: The id of the phase from the linked document or . If phaseType is set to or the phaseId must be set to . If phaseType is set to and phaseId is the phase will reference to the last phase from the linked document.
    void SetPhaseFilter(LinkVisibility phaseFilterType, ElementId phaseFilterId)
      ...
      @phaseFilterId: The id of the phase filter from the linked document or . If phaseFilterType is set to or the phaseFilterId must be set to . If phaseFilterType is set to and phaseFilterId is the state of the phase filter will correspond to the None option in the UI.
    void SetViewDetailLevel(LinkVisibility viewDetailLevelType, ViewDetailLevel viewDetailLevel)
      ...
      @viewDetailLevel: The view detail level. If viewDetailLevelType is set to or the viewDetailLevel must be set to .
...
Remarks: Revolved faces are defined by a profile curve and a local coordinate system. For details on the parameterization, refer to the documentation for
Inherits: Face
Implements: IDisposable
...
Implements: IDisposable
  ...
  METHODS:
    ...
    static Surface Create(XYZ axisBasePoint, XYZ axisDirection, Curve profileCurve, double startAngle, double endAngle)
      ...
      @axisBasePoint: The base point of the axis of revolution. Expected to lie within the Revit design limits .
...
Remarks: A ruled surface is created by sweeping a line between two profile curves or between a curve and a point (a point and a curve). For details on the parameterization, refer to the documentation for .
Inherits: Face
Implements: IDisposable
