...
[ENUM] EditModeType
Full Name: Autodesk.Revit.DB.EditModeType
Description: Identifies the possible edit modes that could be active in Revit.
Inherits: Enum
  ...
  Values:
    ...
    - SpliceRebars = 31
    - ViewPosition = 32
    - AnalyticalAutomation = 33
    - FreeFormRebarEditMode = 34
...
Implements: IComparable
  ...
  METHODS:
    ...
    int CompareTo(object id)
      Description: Compares two element ids. Implementation of System.IComparable interface
      @id: The ElementId to be compared with this ElementId.
      Returns: -1 if this element id is less than other id, 0 if equal, 1 if greater.
...
Inherits: Element
  ...
  METHODS:
    ...
    [SKIPPED METHOD] GetPreviewImage: Could not load file or assembly 'System.Drawing.Common, Version=10.0.0.0, Culture=neutral, PublicKeyToken=cc7b13ffcd2ddd51'. O sistema não pode encontrar o arquivo especificado.
...
Full Name: Autodesk.Revit.DB.EmbodiedCarbonAsset
Description: Represents the properties of a material pertinent to embodied carbon calculations.
Implements: IDisposable

  CONSTRUCTORS:
    new EmbodiedCarbonAsset(string name)
      Description: Constructs an instance of EmbodiedCarbonAsset.
      @name: The name of the asset.
      Throws ArgumentNullException: A non-optional argument was null

  PROPERTIES:
    double CarbonCoefficient { get; }
      Description: Used by Insight, unit is kgCOâ‚‚e per length, area, etc., e.g 200.234 kgCOâ‚‚e/m
    EmbodiedCarbonCoefficientUnitType CarbonCoefficientUnitType { get; }
      Description: The unit type of Embodied Carbon coefficient (e.g. kgCOâ‚‚e/m, kgCOâ‚‚e/kg,...)
    string CategoryId { get; }
      Description: Internal identifier of category. List of available categories can be retrieved from Sustainability API.
    string CategoryName { get; }
      Description: User visible name of category. List of available categories can be retrieved from Sustainability API.
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
    string Name { get; set; }
      Description: The name of the EmbodiedCarbon asset.
    string ProviderId { get; }
      Description: Internal identifier of provider, which provide embodied carbon data. List of available providers can be retrieved from Sustainability API.
    string ProviderName { get; }
      Description: User visible name of provider. List of available providers can be retrieved from Sustainability API.

  METHODS:
    void Dispose()
    bool EqualsEmbodiedCarbonOnly(EmbodiedCarbonAsset other)
      Description: Determines whether this embodied carbon asset is equal to another, but ignore data from base class.
      @other: The embodied carbon asset to compare with this one.
      Returns: True if the given embodied carbon asset is equal to this one, otherwise false.
      Throws ArgumentNullException: A non-optional argument was null

--------------------------------------------------------------------------------

[ENUM] EmbodiedCarbonCoefficientUnitType
Full Name: Autodesk.Revit.DB.EmbodiedCarbonCoefficientUnitType
Description: Represents the unit type of Embodied Carbon coefficient in a EmbodiedCarbon asset.
Inherits: Enum
...
Description: This geometry utility allows you to attempt to "fit" a given piece of solid geometry into the shape of an extrusion.
Remarks: An instance of this class is a single-time use class which should be provided a solid geometry, a plane, and a direction. The utility will calculate a base boundary parallel to the input plane which is the outer boundary of the shadow cast by the Solid onto the input plane and along the extrusion direction.After the extrusion has been calculated, the class permits a second step analysis to identify all faces from the original geometry which do not align with the faces of the calculated extrusion.This utility works best for a Solid that is at least somewhat "extrusion-like", for example, the geometry of a wall which may or may not be affected by end joins, floor joins, roof joins, openings cut by windows and doors, or other modifications.The ExtrusionAnalyzer expects only one single extrusion-like solid geometry. The ExtrusionAnalyzer does not support a Solid containing multiple distinguishable "lumps" of geometry and may give unexpected results in such cases. In such cases, consider splitting the Solid into its individual connected commponent using Autodesk::Revit::DB::SolidUtils::SplitVolumes before performing an extrusion analysis.
Implements: IDisposable
  ...
  METHODS:
    ...
    static ExtrusionAnalyzer Create(Solid solidGeometry, Plane plane, XYZ direction)
      ...
      @solidGeometry: The Solid to analyze.
