...
[ENUM] FreeFormSetOrientationOptions
Full Name: Autodesk.Revit.DB.Structure.FreeFormSetOrientationOptions
Description: Orientation options for Aligned or 3D Path Free Form Rebar set.
Inherits: Enum
...
Implements: IExternalServer
  ...
  METHODS:
    ...
    bool GetCustomHandles(RebarHandlesData handlesInfoData)
      Description: This function should define all handles that the Rebar has.
...
Inherits: LoadBase
  ...
  METHODS:
    static LineLoad Create(Document document, ElementId hostElemId, Curve curve, XYZ forceVector1, XYZ momentVector1, LineLoadType symbol)
      ...
      Throws ArgumentException: The element hostElemId does not exist in the document -or- hostElemId is not permitted for this type of load. -or- The provided curve is not supported. -or- The input curve is not bound. -or- hostElemId is not a valid host for line load.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    static LineLoad Create(Document document, ElementId hostElemId, int curveIndex, XYZ forceVector1, XYZ momentVector1, LineLoadType symbol)
      ...
      @symbol: The symbol of the LineLoad. Set to use default type.
      Returns: If successful, returns the newly created LineLoad, otherwise.
      Throws ArgumentException: The element hostElemId does not exist in the document -or- hostElemId is not permitted for this type of load. -or- hostElemId is not a valid host for line load.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentsInconsistentException: Thrown when all force and moment vectors are equal zero.
      Throws InvalidOperationException: Thrown if type could not be set for newly created line load.
    ...
    static bool IsCurveInsideHostBoundaries(Document doc, ElementId hostId, Curve curve)
      ...
      Throws ArgumentNullException: A non-optional argument was null
    static bool IsValidHostId(Document doc, ElementId hostId)
      Description: Indicates if the provided host id can host line loads.
      @doc: The document containing both the host and the load.
      @hostId: The id of the analytical element that is about to host a line load.
      Returns: True if a line load can be placed on the input host id.
      Throws ArgumentNullException: A non-optional argument was null
    void SetCurve(Curve curve)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: This LineLoad is not a hosted object. -or- This LineLoad is constrained on host.
...
Inherits: Element
  ...
  METHODS:
    bool IsOrientToPermitted(LoadOrientTo orientTo)
      ...
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static bool IsValidReference(Document doc, Reference reference, LoadType type, bool isConstrainedOnHost)
      Description: Indicates if the provided reference can host a load.
      @doc: The document containing both the host and the load.
      @reference: A reference to a analytical element.
      @type: The type of the load that is about to be hosted.
      @isConstrainedOnHost: A value indicating if the load is constrained on host or not.
      Returns: True if a load can be placed on the input host.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    void RemoveHostConstraint()
      Description: Removes constraint from host for this load.
      Throws ModificationForbiddenException: the Load is an AreaLoad and the host is an Analytical Panel with curved profile.
...
Inherits: Element
  ...
  METHODS:
    static LoadCase Create(Document document, string name, ElementId natureId, ElementId subcategoryId)
      ...
      Throws ArgumentException: The given name is not unique. -or- name cannot include prohibited characters, such as "{, }, [, ], |, ;, less-than sign, greater-than sign, ?, `, ~".
      Throws ArgumentNullException: A non-optional argument was null
...
[ENUM] MorphedDistributionRebarHandles
Full Name: Autodesk.Revit.DB.Structure.MorphedDistributionRebarHandles
Description: This enum represents the values that custom handles tags of RebarConstrainedHandle of Rebar Free Form Morphed Distribution have. The tag of a Autodesk::Revit::DB::Structure::RebarHandleType::CustomHandle RebarConstrainedHandle can be obtained by calling the Autodesk::Revit::DB::Structure::RebarConstrainedHandle::GetCustomHandleTag
Inherits: Enum
...
[ENUM] Path3DDistributionRebarHandles
Full Name: Autodesk.Revit.DB.Structure.Path3DDistributionRebarHandles
Description: This enum represents the values that custom handles tags of RebarConstrainedHandle of Rebar Free Form 3D Path Distribution have. The tag of a Autodesk::Revit::DB::Structure::RebarHandleType::CustomHandle RebarConstrainedHandle can be obtained by calling the Autodesk::Revit::DB::Structure::RebarConstrainedHandle::GetCustomHandleTag
Inherits: Enum
...
Inherits: LoadBase
  ...
  METHODS:
    static PointLoad Create(Document document, ElementId hostElemId, XYZ point, XYZ forceVector, XYZ momentVector, PointLoadType symbol)
      ...
      Throws ArgumentException: hostElemId is not permitted for this type of load. -or- Thrown when work plane is not valid. -or- hostElemId is not a valid host for point load.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    static PointLoad Create(Document document, ElementId hostElemId, AnalyticalElementSelector selector, XYZ forceVector, XYZ momentVector, PointLoadType symbol)
      ...
      @symbol: The symbol of the PointLoad. Set to use default type.
      Returns: If successful, returns the newly created PointLoad, otherwise.
      Throws ArgumentException: The element hostElemId does not exist in the document -or- hostElemId is not permitted for this type of load. -or- hostElemId is not a valid host for point load.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
      Throws ArgumentsInconsistentException: Thrown when force and moment vectors are equal zero.
      Throws InvalidOperationException: Thrown if type could not be set for newly created point load.
    static bool IsPointInsideHostBoundaries(Document pDoc, ElementId hostId, XYZ point)
      ...
      Throws ArgumentNullException: A non-optional argument was null
    static bool IsValidHostId(Document doc, ElementId hostId)
      Description: Indicates if the provided host id can host point loads.
      @doc: The document containing both the host and the load.
      @hostId: The id of the analytical element that is about to host a point load.
      Returns: True if a point load can be placed on the input host id.
      Throws ArgumentNullException: A non-optional argument was null
...
Inherits: Element
  ...
  PROPERTIES:
    ...
    static Guid FreeForm3DPathServerGuid { get; }
      Description: Guid used for identifying the external IRebarUpdateServer for computing 3D Path Free Form Rebar.
    ...
    static Guid FreeFormMorphedServerGuid { get; }
      Description: Guid used for identifying the external IRebarUpdateServer for computing Morphed Free Form Rebar.
    ...
    double Mass { get; }
      Description: Identifies the mass of the rebar set. It is computed as the sum of the masses of all individual bars.
  ...
  METHODS:
    static bool AreBarIndicesValidForSplit(Document document, ElementId elementIdToCheck, ISet<int> barIndices)
      Description: Verifies that all bar indices in the input set are within the valid range of [0, NumberOfBarPositions - 2].For a 3D Path or Morphed Free Form Rebar, single layout Rebar sets cannot result from a split, so the bar indices in the input set must not be consecutive.For a 3D Path Free Form Rebar, this also validates that each new set created by the split will have at least one bar plane intersecting a path edge.
      @document: The document.
      @elementIdToCheck: The elementId to check.
      @barIndices: The set of bar indices to validate.
      Returns: Returns true if the validation is successful, false otherwise.
      Throws ArgumentException: Free Form Rebars other than 3D Path or Morphed, rebar sets with only one bar included or rebars that are part of a group can't be split. Morphed Free Form Rebars that targets Spiral or Lapped Circle shapes can't be split.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    static bool CanElementBeSplit(Document document, ElementId elementIdToCheck)
      Description: Checks if the element is a rebar that can be split.
      @document: The document
      @elementIdToCheck: The elementId that need to be checked
      Returns: Returns true if the element is a Rebar that can be split, false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    static IList<double> ComputeSpacingsBetweenBars(Document document, string customSpacingFormula, double distributionLength)
      Description: Computes the spacings between all bars as we would have a set with CustomSpacing layout, the customSpacingFormula and its length being distributionLength.
      @document: A document.
      @customSpacingFormula: The formula to be used to compute spacings.
      @distributionLength: The length along which the bars will be distributed. This length will be used only if the formula contains "?" or percentages.
      Returns: Returns the spacings between all bars in set. If the bars doesn't fit in the input distributionLength, the returned array will be empty.
      Throws ArgumentException: The specified formula is not valid (doesn't have a valid syntax).
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: The given value for distributionLength must be positive.
    ...
    static Rebar CreateFreeForm(Document doc, Guid serverGUID, RebarBarType barType, Element host)
      ...
      @doc: A Document.
      @serverGUID: The API server GUID. Should be the same that the function GetServerId() from class derived from Autodesk::Revit::DB::Structure::IRebarUpdateServer returns. This server has the responsibility to calculate the bars of Rebar.
      @barType: A RebarBarType element that defines bar diameter, bend radius and material of the rebar.
      @host: The element to which the rebar belongs. The element must support rebar hosting.
    ...
    string GetLayoutFormula(bool formatFormula)
      Description: Gets the formula that is used to compute the spacings between bars.
      @formatFormula: If it's false the returned string will contain values that are not rounded and they are in internal Revit units (decimal feet), otherwise it will be formatted according to project units.
      Returns: Returns the formula that is used to compute the spacings between bars.
      Throws InvalidOperationException: Rebar should have CustomSpacing layout.
    ...
    ParameterValue GetParameterValueAtIndex(ElementId paramId, int barPositionIndex)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: barPositionIndex should be in range [ 0, NumberOfBarPositions-1 ] and the bar at barPositionIndex should be included.
    IDictionary<ElementId, int> GetPresentationBarIndicesForSpliceChain(int barIndex, bool bNoShift)
      Description: Given a bar index on this rebar, returns the corresponding bar index on each rebar in the splice chain.
      @barIndex: The starting bar index on this rebar.
      @bNoShift: If true, the distance filter and direction logic are skipped.
      Returns: A map from each rebar element id in the chain to its corresponding bar index.
    ...
    IList<double> GetSpacingsBetweenBars()
      Description: Gets the spacings between all bars in set that have CustomSpacing layout. It will consider that there are no moved or removed bars from the set.
      Returns: Returns the spacings between all bars in set.
      Throws InvalidOperationException: Rebar should have CustomSpacing layout. -or- The Free Form Rebar doesn't have set a distribution path.
    ...
    bool IsCustomSpacingWithPredefinedNumberOfBars()
      Description: Checks if the layout is defined as a Custom Spacing Layout with a predefined number of bars. Custom Spacing layout will have a predefined number of bars if the formula is fixed (not variable and without percentage members).
      Returns: Returns true, if the Layout is Custom Spacing and it has a predefined number of bars(the formula is fixed). It will return false if the layout is other than Custom Spacing or if the formula is variable or has percentage members.
    ...
    static bool IsValidCustomSpacingFormula(Document document, string formula)
      Description: Checks if the specified formula is valid (has a valid syntax).
      @document: A document.
      @formula: The formula to check.
      Returns: Returns true if the specified formula is valid (has a valid syntax), false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    void SetHostId(Document doc, ElementId hostId)
      ...
      Throws ArgumentNullException: A non-optional argument was null
    void SetLayoutFormula(string customSpacingFormula)
      Description: Sets the specified formula for custom spacing.
      @customSpacingFormula: The formula that will be set.
      Throws ArgumentException: The specified formula is not valid (doesn't have a valid syntax). -or- The formula determines an array length of the Rebar set that is not between (0, 10000] feet.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: Rebar should have CustomSpacing layout.
    ...
    void SetUnobscuredInView(View view, bool unobscured)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InapplicableDataException: This rebar element doesn't have valid visibility data.
    static IList<ElementId> SplitRebar(Document document, ElementId rebarToSplit, ISet<int> barIndices, bool constrainSplitSets, bool splitAllSetsInSpliceChain)
      Description: Splits a rebar into multiple sets at specified bar indices. Given split indices (e.g., {ii, jj}) in barIndices, the function creates three rebar sets: from 0 to ii, ii+1 to jj, and jj+1 to NumberOfBarPositions - 1. Edge Case: A split at index 0 creates a first set with one bar; a split at the last index does nothing, as no bars follow. If a split is made at an index where the bar is not included, the split will still occur, and the unincluded bar will remain unchanged in the new set created after the split. This method applies to a Shape Driven Rebar whose definition is RebarShapeDefinitionBySegments, for 3D Path Free Form Rebar and for Morphed Free Form Rebar. For Morphed Free Form Rebar that targets Spiral or Lapped Circle shapes, split cannot be made.
      @document: The document.
      @rebarToSplit: The rebar that will be split
      @barIndices: The array storing the indices of the initial bars from the rebar where the splits will occur.
      @constrainSplitSets: If this is true, Rebar Plane of the split set with index ii in the output array will be constrained to Out of Plane of the set split set with index ii-1 in the output array (or Rebar Plane if the layout is Single). If it is false, it will be no constraint between the resulted sets. This parameter has no effect for 3D Path free form rebars.
      @splitAllSetsInSpliceChain: If this is true, all sets in the splice chain will be split and the splice relationships will be maintained. If it is false, only the selected set will be split and no splice relationship will be preserved. This parameter has no effect for 3D Path free form rebars.
      Returns: Returns the ids of the rebars that are considerred to be split.
      Throws ArgumentException: Free Form Rebars other than 3D Path or Morphed, rebar sets with only one bar included or rebars that are part of a group can't be split. Morphed Free Form Rebars that targets Spiral or Lapped Circle shapes can't be split.
      Throws ArgumentNullException: A non-optional argument was null
...
Inherits: ElementType
  ...
  PROPERTIES:
    double BarMassPerUnitLength { get; set; }
      Description: Defines bar mass per unit length of rebar.
...
Implements: IDisposable
  ...
  METHODS:
    bool AllowsConstraintToCustomTarget()
      Description: Checks if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToCustomTarget type.
      Returns: Returns true if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToCustomTarget type.
    bool AllowsConstraintToHostEdge()
      Description: Checks if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdge type.
      Returns: Returns true if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdge type.
    bool AllowsConstraintToHostEdgeCover()
      Description: Checks if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdgeCover type.
      Returns: Returns true if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdgeCover type.
    bool AllowsConstraintToHostFaceOrToCover()
      Description: Checks if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::FixedDistanceToHostFace or Autodesk::Revit::DB::Structure::RebarConstraintType::ToCover type.
      Returns: Returns true if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::FixedDistanceToHostFace or Autodesk::Revit::DB::Structure::RebarConstraintType::ToCover type.
    bool AllowsConstraintToOtherRebarEdge()
      Description: Checks if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToOtherRebar type, where target is Autodesk::Revit::DB::Structure::RebarConstraintType::TargetRebarConstraintType::Edge.
      Returns: Returns true if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToOtherRebar type, where target is Autodesk::Revit::DB::Structure::RebarConstraintType::TargetRebarConstraintType::Edge.
    bool AllowsConstraintToSurface()
      Description: Checks if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToSurface type.
      Returns: Returns true if the RebarConstrainedHandle allows constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToSurface type.
    bool AllowsDistanceToHostEdgeOrHostEdgeCover()
      Description: Checks if the RebarConstrainedHandle allows distance for a constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdge or Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdgeCover type.
      Returns: Returns true if the RebarConstrainedHandle allows distance for a constraint of Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdge or Autodesk::Revit::DB::Structure::RebarConstraintType::ToHostEdgeCover type, false otherwise.
      Throws InvalidOperationException: The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.ToHostEdge or RebarConstraintType.ToHostEdgeCover type.
    ...
    int GetEdgeNumber()
      ...
      Returns: Returns the number of the edge that is driven by the handle.
    ...
    XYZ GetHandlePosition(View view)
      Description: Gets the position of this handle as it is while editing constraints.
      @view: The view in which the position is requested.
      Returns: Returns the position of this handle as it is while editing constraints.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: RebarConstrainedHandle is no longer valid.
    Surface GetHandleSurface()
      Description: Gets the surface of this handle.
      Returns: Returns the surface of this handle.
      Throws InvalidOperationException: RebarConstrainedHandle is no longer valid. -or- This handle doesn't belong to a shape driven rebar.
    ...
    RebarConstraintsManager GetRebarConstraintsManager()
      Description: Gets the RebarConstraintsManager that is associated with this handle.
      Returns: Return the RebarConstraintsManager that is associated with this handle.
    bool IsCustomHandle()
      Description: Returns true if the RebarHandleType of the RebarConstrainedHandle is 'CustomHandle'.
    bool IsDraggable()
      Description: Checks that the RebarConstrainedHandle can be draggable in canvas (it has drag controls while editing constraints).
      Returns: Returns true if the RebarConstrainedHandle can be draggable in canvas (it has drag controls while editing constraints).
    bool IsEdgeHandle()
    ...
    void Move(XYZ translation)
      Description: Moves the handle with the specified vector.
      @translation: The translation vector that will be applied.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: RebarConstrainedHandle is no longer valid. -or- The RebarConstrainedHandle cannot be dragged.
...
Implements: IDisposable
  ...
  METHODS:
    ...
    static RebarConstraint Create(RebarConstrainedHandle handle, IList<Reference> targetHostFaceReferences, bool isConstraintToCover, double offsetValue)
      Description: This method creates a constraint of 'ToHostFace' or 'ToCover' type for a given RebarConstrainedHandle. Using this method for Shape-Driven Rebar will throw exception. If you need a new RebarConstraint for a handle of a Shape-Driven Rebar, you can obtain a list of possible constraints using one of the RebarConstraintsManager.GetConstraintCandidatesForHandle functions, choose the desired one, and set it as preferred.
      @handle: The handle of the rebar that will be constrained. This handle should not be allowed to constrain to an edge.
      @targetHostFaceReferences: The references to which the rebar handle will be constrained. This collection must contain one or more references to faces of elements that can host rebar.
      ...
      Throws ArgumentException: Constrained rebar isn't a Free Form Rebar element. -or- handle is no longer valid. -or- targetHostFaceReferences is empty. -or- The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.FixedDistanceToHostFace or RebarConstraintTypeToCover type. -or- targetHostFaceReferences do not represent faces from structurals that can host rebar.
      Throws ArgumentNullException: A non-optional argument was null
    static RebarConstraint CreateConstraintToCustomTarget(RebarConstrainedHandle handle, double distanceToCustomTarget)
      Description: This method creates a constraint of 'ToCustomTarget' type for a given RebarConstrainedHandle.The RebarConstrainedHandle should belong to a Free Form Rebar should allow constrains to custom target.
      @handle: The handle of the rebar that will be constrained.
      @distanceToCustomTarget: The distance to the custom target.
      Returns: Returns the newly created RebarConstraint.
      Throws ArgumentException: handle is no longer valid. -or- The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.ToCustomTarget type.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: The given value for distanceToCustomTarget must be no more than 30000 feet in absolute value.
    static RebarConstraint CreateConstraintToHostEdge(RebarConstrainedHandle handle, IList<Reference> targetReferences)
      Description: This method creates a constraint of 'ToHostEdge' type for a given RebarConstrainedHandle.The RebarConstrainedHandle should belong to a Free Form Rebar and must be allowed to constrain to host edges.
      @handle: The handle of the rebar that will be constrained.
      @targetReferences: The references to which the rebar handle will be constrained. This collection must contain one or more references to edges of an element that can host rebar.
      Returns: Returns the newly created RebarConstraint.
      Throws ArgumentException: handle is no longer valid. -or- targetReferences is empty. -or- The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.ToHostEdge type. -or- targetReferences do not represent edges from the same structural that can host rebar.
      Throws ArgumentNullException: A non-optional argument was null
    static RebarConstraint CreateConstraintToHostEdgeCover(RebarConstrainedHandle handle, IList<Reference> targetReferences)
      Description: This method creates a constraint of 'ToHostEdgeCover' type for a given RebarConstrainedHandle.The RebarConstrainedHandle should belong to a Free Form Rebar and must be allowed to constrain to host edges.The RebarConstrainedHandle that is constrained 'ToHostEdgeCover' must allow offset value.
      @handle: The handle of the rebar that will be constrained.
      @targetReferences: The references to which the rebar handle will be constrained. This collection must contain one or more references to edges of an element that can host rebar.
      Returns: Returns the newly created RebarConstraint.
      Throws ArgumentException: handle is no longer valid. -or- targetReferences is empty. -or- The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.ToHostEdgeCover type. -or- targetReferences do not represent edges from the same structural that can host rebar.
      Throws ArgumentNullException: A non-optional argument was null
    static RebarConstraint CreateConstraintToRebarEdge(RebarConstrainedHandle handle, IList<ConstraintToRebarEdgeProperties> allEdgesProperties)
      Description: This method creates a constraint of 'ToOtherRebar' type with Edge target type for a given RebarConstrainedHandle.The RebarConstrainedHandle should belong to a Free Form Rebar, otherwise it will throw exception.
      @handle: The handle of the rebar that will be constrained.
      @allEdgesProperties: An array containing all target edges to which will be constrained and the constraint properties for each target edge.
      Returns: Returns the newly created RebarConstraint.
      Throws ArgumentException: handle is no longer valid. -or- Constrained rebar isn't a Free Form Rebar element. -or- The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.ToOtherRebar type, where target is TargetRebarConstraintType.Edge"/>. -or- All targets(TargetRebarEdge) should be different.
      Throws ArgumentNullException: A non-optional argument was null
    static RebarConstraint CreateConstraintToSurface(RebarConstrainedHandle handle, Surface surface)
      Description: This method creates a constraint of 'ToSurface' type for a given RebarConstrainedHandle.The RebarConstrainedHandle should belong to a Shape Driven Rebar, otherwise it will throw exception.
      @handle: The handle of the rebar that will be constrained.
      @surface: The surface that will be used by the constraint.
      Returns: Returns the newly created RebarConstraint.
      Throws ArgumentException: handle is no longer valid. -or- -or- The RebarConstrainedHandle doesn't allow constraint of RebarConstraintType.ToSurface type.
      Throws ArgumentNullException: A non-optional argument was null
    void Dispose()
    void FlipHandleOverTarget()
      ...
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- Constrained rebar isn't a Shape Driven Rebar element. -or- The TargetRebarConstraintType is 'HookBend' or 'BarBend'.
    void FlipSideForClearBarSpacingZeroDistanceConstraint()
      ...
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- Constrained rebar isn't a Shape Driven Rebar element. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- The RebarConstraint constrain two Rebar ends. -or- The TargetRebarConstraintType is 'HookBend' or 'BarBend'. -or- The RebarConstraint is not using clear bar distance with zero offset to the target rebar.
    IList<ConstraintToRebarEdgeProperties> GetConstraintToRebarEdgeProperties()
      Description: Gets the properties for all target rebar edges.
      Returns: Returns the properties for all target rebar edges.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The TargetRebarConstraintType should be 'Edge'.
    ...
    double GetDistanceToCustomTarget()
      Description: Gets the distance from the RebarConstrainedHandle to the custom target.
      Returns: Returns the distance from the RebarConstrainedHandle to the custom target.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraintType of the RebarConstraint must be 'ToCustomTarget'.
    ...
    double GetDistanceToTargetHostEdge()
      Description: Returns the distance from the RebarConstrainedHandle to the target host edge. The RebarConstraintType of the RebarConstraint must be 'ToHostEdge' and must allow offset value.
      Returns: Returns the distance from the RebarConstrainedHandle to the target host edge.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdge'. -or- The RebarConstrainedHandle doesn't allow distance for constraint of ToHostEdge or ToHostEdgeCover type.
    double GetDistanceToTargetHostEdgeCover()
      Description: Returns the distance from the RebarConstrainedHandle to the target host edge cover. The RebarConstraintType of the RebarConstraint must be 'ToHostEdgeCover'.
      Returns: Returns the distance from the RebarConstrainedHandle to the target host edge cover.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdgeCover'. -or- The RebarConstrainedHandle doesn't allow distance for constraint of ToHostEdge or ToHostEdgeCover type.
    ...
    XYZ GetPositiveOffsetDirectionForToOtherRebarConstraint()
      Description: Returns the default positive offset direction vector for a constraint to a Shape Driven Rebar.
      Returns: Returns the default positive offset direction vector.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- Constrained rebar isn't a Shape Driven Rebar element. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- The TargetRebarConstraintType is not valid.
    ...
    RebarCoverType GetTargetCoverType(int targetIndex)
      ...
      Throws ArgumentException: targetIndex is out of range.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'FixedDistanceToHostFace' or 'ToCover'.
    RebarCoverType GetTargetCoverTypeForToHostEdgeConstraint(int targetIndex, int faceIndex)
      Description: Returns the RebarCoverType for the face provided at faceIndex, that forms the edge of this RebarConstraint at the specified targetIndex. The RebarConstraintType of the RebarConstraint must be 'ToHostEdge' or 'ToHostEdgeCover'. Returns null if no RebarHostData is present for target element.
      @targetIndex: The index of the target. Should be between 0 and NumberOfTargets().
      @faceIndex: The face index of the edge. Should be 0 or 1.
      Returns: Returns the RebarCoverType for the face provided at faceIndex, that forms the edge of this RebarConstraint at the specified targetIndex.
      Throws ArgumentException: targetIndex is out of range. -or- faceIndex must be 0 or 1.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdge' or 'ToHostEdgeCover'.
    Element GetTargetElement()
      Description: Gets the Element object (either Host or Rebar) which provides the target reference at index 0 for this constraint. Will throw exception if it's a multi target constraint.
      Returns: Returns the Element object (either Host or Rebar) which provides the target reference at index 0 for this constraint.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- Multi target constraint. Consider using the indexed version of the method. -or- The RebarConstraint should not be of RebarConstraintType 'ToSurface'. -or- The RebarConstraintType of the RebarConstraint is 'ToCustomTarget'.
    Element GetTargetElement(int targetIndex)
      Description: Gets the Element object (either Host or Rebar) which provides the target reference at specified index for this constraint.
      @targetIndex: The index of the target. Should be between 0 and NumberOfTargets().
      Returns: Returns the Element object (either Host or Rebar) which provides the target reference at specified index for this constraint.
      Throws ArgumentException: targetIndex is out of range.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint should not be of RebarConstraintType 'ToSurface'. -or- The RebarConstraintType of the RebarConstraint is 'ToCustomTarget'.
    TargetHostEdgeAndTransform GetTargetHostEdgeAndTransform(int targetIndex)
      Description: Returns the target host edge and the transform that is associated to the host edge's geometry. The RebarConstraintType of the RebarConstraint must be 'ToHostEdge' or 'ToHostEdgeCover'. Only applied to Free Form Rebar.
      @targetIndex: The index of the target. Should be between 0 and NumberOfTargets().
      Returns: Returns the target host edge and the transform that is associated to the host edge's geometry.
      Throws ArgumentException: targetIndex is out of range.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdge' or 'ToHostEdgeCover'. -or- Constrained rebar isn't a Free Form Rebar element.
    ...
    Reference GetTargetHostFaceReference(int targetIndex)
      ...
      Throws ArgumentException: targetIndex is out of range.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'FixedDistanceToHostFace' or 'ToCover'.
    Reference GetTargetHostFaceReferenceForEdgeConstraint(int targetIndex, int faceIndex)
      Description: Returns a reference to the host Element face provided at faceIndex, that forms the edge of this RebarConstraint at the specified targetIndex. The RebarConstraintType of the RebarConstraint must be 'ToHostEdge' or 'ToHostEdgeCover'.
      @targetIndex: The index of the target. Should be between 0 and NumberOfTargets().
      @faceIndex: The face index of the edge. Should be 0 or 1.
      Returns: Returns a reference to the host Element face provided at faceIndex, that forms the edge of this RebarConstraint at the specified targetIndex.
      Throws ArgumentException: targetIndex is out of range. -or- faceIndex must be 0 or 1.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdge' or 'ToHostEdgeCover'.
    int GetTargetRebarAngleOnBarOrHookBend()
      ...
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- The TargetRebarConstraintType is not 'HookBend' or 'BarBend'. -or- Constrained rebar isn't a Shape Driven Rebar element.
    int GetTargetRebarBendNumber()
      Description: Gets the number of the bend on the other Rebar Element to which this RebarConstraint is attached. The RebarConstraint must be of RebarConstraintType 'ToOtherRebar,' and the TargetRebarConstraintType must be 'BarBend'. Rebar must be Shape Driven Rebar element.
      Returns: Returns the number of the bend on the other Rebar Element to which this RebarConstraint is attached.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- The TargetRebarConstraintType is not 'BarBend' -or- Constrained rebar isn't a Shape Driven Rebar element.
    TargetRebarConstraintType GetTargetRebarConstraintType()
      Description: Gets the TargetRebarConstraintType of the handle on the other Rebar Element to which this RebarConstraint is attached. The RebarConstraintType of the RebarConstraint must be 'ToOtherRebar'.
      Returns: Returns the TargetRebarConstraintType of the handle on the other Rebar Element to which this RebarConstraint is attached.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'.
    int GetTargetRebarEdgeNumber()
      Description: Gets the number of the edge on the other Rebar Element to which this RebarConstraint is attached. The RebarConstraint must be of RebarConstraintType 'ToOtherRebar,' and the TargetRebarConstraintType must be 'Edge'. Rebar must be Shape Driven Rebar element.
      Returns: Returns the number of the edge on the other Rebar Element to which this RebarConstraint is attached.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- Constrained rebar isn't a Shape Driven Rebar element. -or- The RebarConstraint is not attached to an edge, or bend, or hook bend of another Rebar Element, or it's attached to multiple edges.
    ...
    int GetTargetRebarHookBarEnd()
      ...
      Returns: Returns 0 or 1 to indicate which end hook on the other Rebar Element to which this RebarConstraint is attached.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- The TargetRebarConstraintType is not 'HookBend' -or- Constrained rebar isn't a Shape Driven Rebar element.
    ...
    static bool IsReferenceAValidEdgeForConstraint(Document document, Reference targetReference)
      Description: Checks if the reference provided can be used in creating 'ToHostEdge' or 'ToHostEdgeCover' constraint.
      @document: The document that contains the reference.
      @targetReference: The reference to be checked.
      Returns: Returns true if reference can be used in a constraint of 'ToHostEdge' or 'ToHostEdgeCover' type, false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
    static bool IsReferenceAValidFaceForConstraint(Document document, Reference targetReference)
      Description: Checks if the reference provided can be used in creating 'ToFace' or 'ToCover' constraints.
      @document: The document that contains the reference.
      @targetReference: The reference to be checked.
      Returns: Returns true if reference can be used in a constraint of 'ToFace' or 'ToCover' type, false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
    bool IsReferenceValidForConstraint(Reference targetReference)
      Description: Checks if the reference provided can be used in creating 'ToFace' or 'ToCover' Rebar constraints
      @targetReference: The reference to be checked.
      Returns: Returns true if reference can be used in a constraint of 'ToFace' or 'ToCover' type, false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: The RebarConstraint is not of RebarConstraintType 'FixedDistanceToHostFace' or 'ToCover'.
    bool IsToCover()
    ...
    bool IsToCustomTarget()
      Description: Checks if the RebarConstraintType of the RebarConstraint is 'ToCustomTarget'.
      Returns: Returns true if the RebarConstraintType of the RebarConstraint is 'ToCustomTarget', false otherwise.
      Throws InvalidOperationException: RebarConstraint is no longer valid.
    bool IsToHostEdge()
      Description: Checks if the RebarConstraintType of the RebarConstraint is 'ToHostEdge'.
      Returns: Returns true if the RebarConstraintType of the RebarConstraint is 'ToHostEdge', false otherwise.
    bool IsToHostEdgeCover()
      Description: Checks if the RebarConstraintType of the RebarConstraint is 'ToHostEdgeCover'.
      Returns: Returns true if the RebarConstraintType of the RebarConstraint is 'ToHostEdgeCover', false otherwise.
    ...
    bool IsUsingClearBarSpacing()
      ...
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToOtherRebar'. -or- Constrained rebar isn't a Shape Driven Rebar element. -or- The RebarConstraint constrain two Rebar ends. -or- The TargetRebarConstraintType is 'HookBend' or 'BarBend'.
    bool IsValid()
    ...
    static bool IsValidSurfaceToConstraintHandleTo(RebarConstrainedHandle handle, Surface surface)
      ...
      Throws ArgumentNullException: A non-optional argument was null
    void ReplaceReferenceTargets(IList<Reference> targetReferences, RebarConstraintType newConstraintType)
      Description: Replaces the current set of references, with the newly provided ones. Will throw exception if this is a constraint for Shape Driven Rebar.
      @targetReferences: The references to which the rebar handle will be constrained. This collection must contain one or more references to faces or edges of elements that can host rebar.
      @newConstraintType: The type of constraint to be set. Can only be 'ToCover', 'FixedDistanceToHostFace', 'ToHostEdge' or 'ToHostEdgeCover'. In case the type is 'ToHostEdge' or 'ToHostEdgeCover' the references must be of edges, otherwise of faces.
      Throws ArgumentException: targetReferences is empty. -or- newConstraintType is not 'ToHostEdge', 'ToHostEdgeCover', 'ToCover' or 'FixedDistanceToHostFace'. -or- targetReferences do not represent edges or faces from structurals that can host rebar. -or- The handle of the constraint do not accept this type of constraint.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
      Throws InvalidOperationException: Constrained rebar isn't a Free Form Rebar element.
    void ReplaceReferenceTargets(RebarConstrainedHandle handle, IList<Reference> targetReferences, bool isConstraintToCover, double offsetValue)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: Constrained rebar isn't a Free Form Rebar element. -or- The RebarConstraint is not of RebarConstraintType 'FixedDistanceToHostFace' or 'ToCover'.
    void SetConstraintToRebarEdgeProperties(ConstraintToRebarEdgeProperties properties)
      Description: Sets the properties of the constraint to target rebar edges.
      @properties: The properties that will be set.
      Throws ArgumentException: The input properties are not for an existing target of the constraint.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The TargetRebarConstraintType should be 'Edge'.
    void SetDistanceToCustomTarget(double distanceToCustomTarget)
      Description: Sets the distance from the RebarConstrainedHandle to the custom target.
      @distanceToCustomTarget: The distance to custom target.
      Throws ArgumentOutOfRangeException: The given value for distanceToCustomTarget must be no more than 30000 feet in absolute value.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraintType of the RebarConstraint must be 'ToCustomTarget'.
    void SetDistanceToTargetCover(double distanceToTargetCover)
      ...
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToCover'.
    void SetDistanceToTargetHostEdge(double distanceToTargetEdge)
      Description: Sets the distance from the RebarConstrainedHandle to the target host edge. The RebarConstraintType of the RebarConstraint must be 'ToHostEdge' and must allow offset value.
      @distanceToTargetEdge: The distance to target host edge.
      Throws ArgumentOutOfRangeException: The given value for distanceToTargetEdge must be no more than 30000 feet in absolute value.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdge'. -or- The RebarConstrainedHandle doesn't allow distance for constraint of ToHostEdge or ToHostEdgeCover type.
    void SetDistanceToTargetHostEdgeCover(double distanceToTargetEdgeCover)
      Description: Sets the distance from the RebarConstrainedHandle to the target host edge cover. The RebarConstraintType of the RebarConstraint must be 'ToHostEdgeCover'.
      @distanceToTargetEdgeCover: The distance to target host edge cover.
      Throws ArgumentOutOfRangeException: The given value for distanceToTargetEdgeCover must be no more than 30000 feet in absolute value.
      Throws InvalidOperationException: RebarConstraint is no longer valid. -or- The RebarConstraint is not of RebarConstraintType 'ToHostEdgeCover'. -or- The RebarConstrainedHandle doesn't allow distance for constraint of ToHostEdge or ToHostEdgeCover type.
    ...
    bool TargetIsBarBend()
      Description: Returns true if the TargetRebarConstraintType of the RebarConstraint is 'BarBend'.
    bool TargetIsHookBend()
      Description: Returns true if the TargetRebarConstraintType of the RebarConstraint is 'HookBend'.
...
Implements: IDisposable
  ...
  PROPERTIES:
    static bool IsRebarConstrainedPlacementEnabled { get; set; }
      Description: A static property defining that the 'Rebar Constrained Placement' setting is on or off in current Revit Application Session.
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
  ...
  METHODS:
    ...
    bool ApplyRebarConstraints(IList<RebarConstraint> constraintsToApply, IList<Reference> oldTargets, IList<Reference> newTargets)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: The RebarConstraintsManager does not manage a valid Rebar element.
    void ClearHandleConstraintPairHighlighting(Document document)
      Description: Clears all highlighting in all views.
      @document: The document in which the highlighting will be cleared.
      Throws ArgumentNullException: A non-optional argument was null
    void Dispose()
    ...
    RebarConstraint GetPreferredConstraintOnHandle(RebarConstrainedHandle handle)
      ...
      Throws InvalidOperationException: The RebarConstraintsManager does not manage a valid Rebar element.
    Rebar GetRebar()
      Description: Gets the Rebar element whose constraints are managed using this instance.
      Returns: Returns the Rebar element whose constraints are managed using this instance.
    bool HasValidRebar()
      Description: Checks whether the Manager's Rebar is still valid.
    void HighlightHandleConstraintPairInAllViews(Document document, RebarConstrainedHandle handle, RebarConstraint constraint)
      Description: Highlights the specified RebarConstrainedHandle and RebarConstraint in all views.
      @document: The document in which the highlighting will be applied.
...
Implements: IEnumerable<RebarContainerItem>, IEnumerable
  ...
  METHODS:
    ...
    RebarContainerItem AppendItemFromRebar(Rebar rebar)
      ...
      @rebar: The Rebar.
      Returns: The Rebar Container Item.
      Throws ArgumentException: A RebarContainerItem cannot be created from a Free Form Rebar or from a Shape Driven Rebar whose Rebar Shape has End Treatments or Cranks or terminations' rotation angles are different than 0. -or- Can't create container from Rebar with moved bars. -or- Can't create container from Rebar with Custom Spacing layout.
      Throws ArgumentNullException: A non-optional argument was null
...
Implements: IDisposable
  ...
  METHODS:
    ...
    void SetFromRebar(Rebar rebar)
      ...
      @rebar: The Rebar.
      Throws ArgumentException: Can't create container from Rebar with moved bars. -or- Can't create container from Rebar which has excluded bars other than the first and last one. -or- A RebarContainerItem cannot be created from a Free Form Rebar or from a Shape Driven Rebar whose Rebar Shape has End Treatments or Cranks or terminations' rotation angles are different than 0. -or- Can't create container from Rebar with Custom Spacing layout.
      Throws ArgumentNullException: A non-optional argument was null
...
Implements: IDisposable
  ...
  METHODS:
    ...
    bool CanAddBarGeometry()
      Description: If the layout rule is Single or FixedNumber or NumberWithSpacing this function will return true if getNumberOfBarGeometry() is less getBarsNumber(), false otherwise.If the layout rule is CustomSpacing, it will return true if GetNumberOfBarGeometry() is less the number of bars (specified by the formula), false otherwise.For the other layout types, will return true.
...
Implements: IDisposable
  ...
  PROPERTIES:
    ...
    FreeFormSetOrientationOptions FreeFormSetOrientationOptions { get; set; }
      Description: Identifies the orientation options for an Aligned or 3D Path Free Form Rebar set.
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
  ...
  METHODS:
    ...
    void Dispose()
    void Flip3dPathSet()
      Description: For a free form set distributed along a 3D path, this method flips the distribution path. The set should not have Single layout rule.
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar is not created with the 3D Path Free Form rebar server. -or- Rebar has Single layout rule.
    ComputationDataFor3DPathSet GetComputationDataFor3DPathSet()
      Description: For a free form set distributed along a 3D path, this method gets the computation data.
      Returns: Returns the computation data.
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar is not created with the 3D Path Free Form rebar server.
    ...
    static XYZ GetDefaultPositiveDirectionForMorphedTargetEdgesHandle(Document document, TargetRebarEdge targetRebarEdge, int barIndex)
      Description: Gets the default positive direction of the specified target rebar edge in terms of how Morphed Free Form Rebar is defining the default positive direction.
      @document: The document that contains the target rebar.
      @targetRebarEdge: The target rebar edge for which it will get the default positive direction.
      @barIndex: The index of the bar in the target set. Should be between 0 and NumberOfBars() - 1.
      Returns: Returns the default positive direction of the specified target rebar edge.
      Throws ArgumentException: The target rebar should be a valid Rebar Target and bar index should be between 0 and NumberOfBars() - 1.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    bool HasValid3DPathServer()
      Description: Returns true if the current rebar is created with the 3D Path Free Form rebar server, false otherwise.
      Returns: Returns true if the current rebar is created with the 3D Path Free Form rebar server, false otherwise.
    ...
    bool HasValidMorphedServer()
      Description: Returns true if the current rebar is created with the Morphed Free Form rebar server, false otherwise.
      Returns: Returns true if the current rebar is created with the Morphed Free Form rebar server, false otherwise.
    bool HasValidServer()
    ...
    bool HasValidSurfaceServer()
      Description: Returns true if the current rebar is created with the Surface Free Form rebar server, false otherwise.
      Returns: Returns true if the current rebar is created with the Surface Free Form rebar server, false otherwise.
    ...
    static bool IsValidRebarTargetForMorphedRebar(Rebar rebarTarget)
      Description: Checks if the rebar can be used as a target for morphed rebar. A rebar is a valid target for morphed rebar if all the bars in set have the same number of segments and all are planar. Multiplanar bars are considered invalid.
      @rebarTarget: The rebar that will be tested.
      Returns: Return true if the rebar is a valid target for morphed rebar, false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
    void RemoveUpdatingSharedParameter(ElementId parameterId)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: This RebarFreeFormAccessor Rebar doesn't contain a valid server GUID. -or- parameterId is a parameter that is not bound to the Rebar element category.
    void SetComputationDataFor3DPathSet(ComputationDataFor3DPathSet computationData)
      Description: For a free form set distributed along a 3D path, this method sets the computation data.
      @computationData: The computation data.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar is not created with the 3D Path Free Form rebar server.
    ...
    RebarFreeFormValidationResult SetCurves(IList<IList<Curve>> curves)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: This RebarFreeFormAccessor Rebar is constrained.
    void SetLayoutAsCustomSpacing(string customSpacingFormula)
      Description: Sets the Layout Rule property of rebar set to Custom Spacing.
      @customSpacingFormula: The layout formula.
      Throws ArgumentException: The specified formula is not valid (doesn't have a valid syntax).
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar doesn't contain a valid server GUID.
    void SetLayoutAsFixedNumber(int numberOfBars)
      ...
      Throws ArgumentOutOfRangeException: the number of bar positions numberOfBars is less than 1 or more than 1002.
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar doesn't contain a valid server GUID.
    void SetLayoutAsMaximumSpacing(double spacing)
      ...
      Throws ArgumentOutOfRangeException: The spacing isn't bigger than 0.0.
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar doesn't contain a valid server GUID.
    ...
    void SetLayoutAsNumberWithSpacing(int numberOfBars, double spacing)
      ...
      Throws ArgumentOutOfRangeException: The spacing isn't bigger than 0.0. -or- the number of bar positions numberOfBars is less than 1 or more than 1002.
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar doesn't contain a valid server GUID.
    void SetLayoutAsSingle()
      Description: Sets the Layout Rule property of rebar set to Single.
      Throws InvalidOperationException: This RebarFreeFormAccessor doesn't contain a valid rebar reference. -or- This RebarFreeFormAccessor Rebar doesn't contain a valid server GUID.
...
Implements: IDisposable
  ...
  METHODS:
    ...
    Rebar GetRebarElement()
      Description: Gets the rebar element that will be updated by the server.
      Returns: Returns the rebar element that will be updated by the server.
    View GetView()
      Description: Gets the view in which the handles position should be computed.
      Returns: Returns the view in which the handles position should be computed.
...
Implements: IDisposable
  ...
  METHODS:
    ...
    void Dispose()
    void FlipRebar()
      Description: Flips the rebar by flipping the ends and the bar segments.
      Throws InvalidOperationException: This RebarShapeDrivenAccessor doesn't contain a valid rebar reference. -or- This RebarShapeDrivenAccessor is an instance of a spiral shape.
    void FlipRebarSet()
    ...
    void SetLayoutAsCustomSpacing(string customSpacingFormula, double arrayLength, bool barsOnNormalSide, bool includeFirstBar, bool includeLastBar)
      Description: Sets the Layout Rule property of rebar set to Custom Spacing.
      @customSpacingFormula: The layout formula.
      @arrayLength: The distribution length of rebar set. This will be used only if the formula contains percentage or it contains "?", otherwise the array length will be calculated from the formula and will be stored in the rebar.
      @barsOnNormalSide: Identifies if the bars of the rebar set are on the same side of the rebar plane indicated by the normal.
      @includeFirstBar: Identifies if the first bar in rebar set is shown.
      @includeLastBar: Identifies if the last bar in rebar set is shown.
      Throws ArgumentException: The specified formula is not valid (doesn't have a valid syntax).
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: the set length arrayLength isn't acceptable.
      Throws InapplicableDataException: This RebarShapeDrivenAccessor is an instance of a spiral or multiplanar shape.
      Throws InvalidOperationException: This RebarShapeDrivenAccessor doesn't contain a valid rebar reference.
    void SetLayoutAsFixedNumber(int numberOfBarPositions, double arrayLength, bool barsOnNormalSide, bool includeFirstBar, bool includeLastBar)
      Description: Sets the Layout Rule property of rebar set to FixedNumber.
...
Description: Class with different utilities for Rebar Splice.
  ...
  METHODS:
    ...
    static XYZ GetLapDirectionForSpliceGeometryAndPosition(Rebar rebar, RebarSpliceGeometry spliceGeometry, RebarSplicePosition splicePosition)
      ...
      Throws ArgumentException: RebarSplicePosition should not be Middle. -or- Free Form Rebars other than Morphed, or Shape Driven Rebars that are Multiplanar or have a shape whose definition is RebarShapeDefinitionByArc can't be spliced. Also, if the Rebar is member of a Group it cannot be spliced. -or- The rebar cannot be spliced with the provided RebarSpliceGeometry.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static IList<ElementId> GetSpliceChain(Rebar rebar)
      ...
      @rebar: The input rebar.
      Returns: Returns all the rebars that are part of a splice chain with the input rebar.
      Throws ArgumentNullException: A non-optional argument was null
    static RebarSpliceByRulesResult GetSpliceGeometries(Document document, ElementId rebarIdToSplice, RebarSpliceOptions spliceOptions, RebarSpliceRules spliceRules)
      Description: Computes a list of RebarSpliceGeometry which respects the rules. This list can be used to splice the Rebar.
      @document: The document
      @rebarIdToSplice: Rebar id to splice.
      ...
      Throws ArgumentNullException: A non-optional argument was null
    static IList<ElementId> SpliceRebar(Document document, ElementId rebarIdToSplice, RebarSpliceOptions spliceOptions, Line line, XYZ linePlaneNormal)
      ...
      Throws ArgumentException: Free Form Rebars other than Morphed, or Shape Driven Rebars that are Multiplanar or have a shape whose definition is RebarShapeDefinitionByArc can't be spliced. Also, if the Rebar is member of a Group it cannot be spliced. -or- The rebar cannot be spliced with the provided line.
      Throws ArgumentNullException: A non-optional argument was null
    static IList<ElementId> SpliceRebar(Document document, ElementId rebarIdToSplice, RebarSpliceOptions spliceOptions, Line line, ElementId viewId)
      Description: Splice a Rebar with a line.
      @document: The document
      @rebarIdToSplice: Rebar id to splice.
      @spliceOptions: The rebar splice options.
      ...
      Throws ArgumentException: Free Form Rebars other than Morphed, or Shape Driven Rebars that are Multiplanar or have a shape whose definition is RebarShapeDefinitionByArc can't be spliced. Also, if the Rebar is member of a Group it cannot be spliced. -or- The rebar cannot be spliced with the provided line.
      Throws ArgumentNullException: A non-optional argument was null
    static IList<ElementId> SpliceRebar(Document document, ElementId rebarIdToSplice, RebarSpliceOptions spliceOptions, IList<RebarSpliceGeometry> spliceGeometries)
      ...
      Returns: Returns the ids of the rebars that are considered to be spliced.
      Throws ArgumentException: Free Form Rebars other than Morphed, or Shape Driven Rebars that are Multiplanar or have a shape whose definition is RebarShapeDefinitionByArc can't be spliced. Also, if the Rebar is member of a Group it cannot be spliced.
      Throws ArgumentNullException: A non-optional argument was null
...
Implements: IDisposable
  ...
  PROPERTIES:
    ...
    bool EndConstraintLostTargets { get; }
      Description: Indicates if the end handle constraint lost one or more targets in the last regeneration (i.e. structural was modified in a way that target faces changed or were removed).
    ...
    bool IsCustomSpacingFormulaChanged { get; }
      Description: Indicates if the custom formula has changed since the last regeneration.
    ...
    bool StartConstraintLostTargets { get; }
      Description: Indicates if the start handle constraint lost one or more targets in the last regeneration (i.e. structural was modified in a way that target faces changed or were removed).
