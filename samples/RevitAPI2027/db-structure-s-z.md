...
Description: This enum represents the values that custom handles tags of RebarConstrainedHandle of Rebar Free Form Surface Distribution have. The tag of a Autodesk::Revit::DB::Structure::RebarHandleType::CustomHandle RebarConstrainedHandle can be obtained by calling the Autodesk::Revit::DB::Structure::RebarConstrainedHandle::GetCustomHandleTag
Inherits: Enum
...
Full Name: Autodesk.Revit.DB.Structure.TargetHostEdgeAndTransform
Description: This class stores the target host edge and the transform that is associated to the host edge's geometry.
Implements: IDisposable
  ...
  METHODS:
    ...
    Transform GetHostEdgeTransform()
      Description: Gets the transform that is associated to the host edge's geometry.
      Returns: Returns the transform that is associated to the host edge's geometry.
    Edge GetTargetHostEdge()
      Description: Gets the target host edge.
      Returns: Returns the target host edge.

--------------------------------------------------------------------------------

[ENUM] TargetRebarConstraintType
Full Name: Autodesk.Revit.DB.Structure.TargetRebarConstraintType
Description: A type used to identify the particular part of a rebar target to which a rebar's handle is constrained.
...
Full Name: Autodesk.Revit.DB.Structure.TargetRebarEdge
Description: This class is used to identify the target edge of a 'ToOtherRebar' RebarConstraint.
Implements: IDisposable

  CONSTRUCTORS:
    new TargetRebarEdge(Rebar targetRebar, int targetEdgeIndex)
      Description: Constructs a new instance of TargetRebarEdge.
      @targetRebar: The target Rebar.
      @targetEdgeIndex: Identifies the index of the target Rebar's edge. The index should be between 0 and centerlines.Count - 1, where centerlines are computed by suppressing hooks, cranks and fillets for the first bar in the target set.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: targetEdgeIndex is not a valid edge index for the rebar. It should be between 0 and centerlines.Count - 1, where centerlines are computed by suppressing hooks, cranks and fillets for the first bar in set.

  PROPERTIES:
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
    int TargetEdgeIndex { get; }
      Description: Identifies the index of the target Rebar's edge. The index is a number between 0 and centerlines.Count - 1, where centerlines are computed by suppressing hooks, cranks and fillets for the first bar in set.
    ElementId TargetRebarId { get; }
      Description: Identifies the id of the target Rebar.
