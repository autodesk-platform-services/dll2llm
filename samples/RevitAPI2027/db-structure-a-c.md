...
[ENUM] AlignedDistributionRebarHandles
Full Name: Autodesk.Revit.DB.Structure.AlignedDistributionRebarHandles
Description: This enum represents the values that custom handles tags of RebarConstrainedHandle of Rebar Free Form Aligned Distribution have. The tag of a Autodesk::Revit::DB::Structure::RebarHandleType::CustomHandle RebarConstrainedHandle can be obtained by calling the Autodesk::Revit::DB::Structure::RebarConstrainedHandle::GetCustomHandleTag
Inherits: Enum
...
Inherits: Element
  ...
  PROPERTIES:
    ...
    ConnectorManager ConnectorManager { get; }
      Description: Retrieves the Connector Manager from this AnalyticalElement.
  ...
  METHODS:
    ...
    void SetTransform(Transform trf)
      ...
      Throws ArgumentException: The provided transformation for Local Coordinate System is not valid.
      Throws ArgumentNullException: A non-optional argument was null
...
Inherits: LoadBase
  ...
  METHODS:
    ...
    static AreaLoad Create(Document document, ElementId hostElemId, IList<CurveLoop> loops, IList<XYZ> forceVectors, IList<int> refPointCurveIndexes, IList<int> refPointCurveEnds, AreaLoadType symbol)
      ...
      Throws ArgumentException: hostElemId is not permitted for this type of load. -or- One of the following requirements is not satisfied : - curve loops loops are not planar - curve loops loops are self-intersecting - curve loops loops contains zero length curves -or- Thrown when force vector is equal zero. -or- hostElemId is not a valid host for area load.
      Throws ArgumentNullException: A non-optional argument was null
    ...
    static AreaLoad Create(Document document, ElementId hostElemId, IList<CurveLoop> loops, XYZ forceVector, AreaLoadType symbol)
      ...
      @symbol: The symbol of the AreaLoad. Set to use default type.
      Returns: If successful, returns an object of the newly created AreaLoad. is returned if the operation fails.
      Throws ArgumentException: hostElemId is not permitted for this type of load. -or- One of the following requirements is not satisfied : - curve loops loops are not planar - curve loops loops are self-intersecting - curve loops loops contains zero length curves -or- hostElemId is not a valid host for area load.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: Thrown if type could not be set for newly created area load.
    static AreaLoad Create(Document document, ElementId hostElemId, XYZ forceVector, AreaLoadType symbol)
      ...
      @symbol: The symbol of the AreaLoad. Set to use default type.
      Returns: If successful, returns an object of the newly created AreaLoad. is returned if the operation fails.
      Throws ArgumentException: The element hostElemId does not exist in the document -or- hostElemId is not permitted for this type of load. -or- Thrown when force vector is equal zero. -or- hostElemId is not a valid host for area load.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: Thrown if type could not be set for newly created area load.
    ...
    static bool IsValidHostId(Document doc, ElementId hostId)
      Description: Indicates if the provided host id can host area loads.
      @doc: The document containing both the host and the load.
      @hostId: The id of the analytical element that is about to host an area load.
      Returns: True if an area load can be placed on the input host id.
      Throws ArgumentNullException: A non-optional argument was null
    bool SetLoops(Document doc, IList<CurveLoop> newLoops)
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidOperationException: This AreaLoad is not a hosted object. -or- This AreaLoad is constrained on host.
...
Full Name: Autodesk.Revit.DB.Structure.BarTerminationsData
Description: Class that stores data about reinforcement's terminations (e.g. hooks, cranks, end treatments)
Implements: IDisposable

  CONSTRUCTORS:
    new BarTerminationsData(Document document)
      Description: Creates a new instance of BarTerminationData with the default values.
      @document: The document containing the hook type ids, end treatment ids and crank type ids.
      Throws ArgumentNullException: A non-optional argument was null

  PROPERTIES:
    ElementId CrankTypeIdAtEnd { get; set; }
      Description: Identifies the crank type at the end of bar. Setting this property to a valid value, will set the HookTypeIdAtEnd and EndTreatmentTypeIdAtEnd to ElementId.InvalidElementId.
    ElementId CrankTypeIdAtStart { get; set; }
      Description: Identifies the crank type at the start of bar. Setting this property to a valid value, will set the HookTypeIdAtStart and EndTreatmentTypeIdAtStart to ElementId.InvalidElementId.
    ElementId EndTreatmentTypeIdAtEnd { get; set; }
      Description: Identifies the end treatment type at the end of bar. Setting this property to a valid value, will set the CrankTypeIdAtEnd and HookTypeIdAtEnd to ElementId.InvalidElementId.
    ElementId EndTreatmentTypeIdAtStart { get; set; }
      Description: Identifies the end treatment type at the start of bar. Setting this property to a valid value, will set the CrankTypeIdAtStart and HookTypeIdAtStart to ElementId.InvalidElementId.
    ElementId HookTypeIdAtEnd { get; set; }
      Description: Identifies the hook type at the end of bar. Setting this property to a valid value, will set the CrankTypeIdAtEnd and EndTreatmentTypeIdAtEnd to ElementId.InvalidElementId.
    ElementId HookTypeIdAtStart { get; set; }
      Description: Identifies the hook type at the start of bar. Setting this property to a valid value, will set the CrankTypeIdAtStart and EndTreatmentTypeIdAtStart to ElementId.InvalidElementId
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
    RebarTerminationOrientation TerminationOrientationAtEnd { get; set; }
      Description: Identifies the orientation of the termination's (e.g. hook, crank) plane at the end of the bar with respect to the orientation of the last curve and the plane normal.Only two values are permitted:Value = Right: The termination is on your right as you stand at the end of the bar, with the bar behind you, taking the bar's normal as "up."Value = Left: The termination is on your left as you stand at the end of the bar, with the bar behind you, taking the bar's normal as "up."The default value is Left.
    RebarTerminationOrientation TerminationOrientationAtStart { get; set; }
      Description: Identifies the orientation of the termination's (e.g. hook, crank) plane at the start of the bar with respect to the orientation of the first curve and the plane normal.Only two values are permitted:Value = Right: The termination is on your right as you stand at the end of the bar, with the bar behind you, taking the bar's normal as "up."Value = Left: The termination is on your left as you stand at the end of the bar, with the bar behind you, taking the bar's normal as "up."The default value is Left.
    double TerminationRotationAngleAtEnd { get; set; }
      Description: Identifies the termination's (e.g. hook, crank) out of plane rotation angle (in radians) at the end of bar.The default value is 0.
    double TerminationRotationAngleAtStart { get; set; }
      Description: Identifies the termination's (e.g. hook, crank) out of plane rotation angle (in radians) at the start of bar.The default value is 0.

  METHODS:
    RebarShapeTerminationsData AsRebarShapeTerminationsData()
      Description: Creates a new instance of RebarShapeTerminationsData from this.
      Returns: Returns a new instance of RebarShapeTerminationsData from this.
    void Dispose()
    Document GetDocument()
      Description: Gets the document containing the hook type ids, end treatment ids and crank type ids.
      Returns: Returns the document containing the hook type ids, end treatment ids and crank type ids.
...
Description: An object that represents a force applied across an area. Data members Create methods Properties
Inherits: Element
  ...
  PROPERTIES:
    ...
    bool IsConstrainedOnHost { get; }
      Description: The boundary conditions is constrained to host or not.
    XYZ Point { get; set; }
      Description: The position of point boundary conditions.
    BoundaryConditionsPreset Preset { get; set; }
      Description: The boundary conditions preset.
    ElementId SketchId { get; }
      Description: Sketch associated to this Revit element.

  METHODS:
    static bool AreCurveLoopsValid(IList<CurveLoop> loops)
      Description: Checks if curve loops are valid for creating an area boundary conditions.
      @loops: The curve loops to be checked.
      Returns: Returns true if curve loops are ok, false otherwise.
      Throws ArgumentNullException: A non-optional argument was null
    static bool CanCreateAreaBoundaryConditions(Document doc, Reference reference, bool isConstrainedOnHost, IList<CurveLoop> loops)
      Description: Checks if the provided host can host an area boundary conditions.
      @doc: The document containing both the host and the boundary conditions.
      @reference: A reference to an analytical element.
      @isConstrainedOnHost: A value indicating if the area boundary conditions is constrained on host or not.
      @loops: The loops that define geometry of the area boundary conditions. This is taken into account only if isConstrainedOnHost is false.
      Returns: True if an area boundary conditions can be placed on the input host.
      Throws ArgumentNullException: A non-optional argument was null
    static bool CanCreateLineBoundaryConditions(Document doc, Reference reference, bool isConstrainedOnHost, Curve curve)
      Description: Checks if the provided host can host a line boundary conditions.
      @doc: The document containing both the host and the boundary conditions.
      @reference: A reference to an analytical element.
      @isConstrainedOnHost: A value indicating if the line boundary conditions is constrained on host or not.
      @curve: Curve of the boundary conditions. This is taken into account only if isConstrainedOnHost is false.
      Returns: True if a line boundary conditions can be placed on the input host.
      Throws ArgumentNullException: A non-optional argument was null
    static bool CanCreatePointBoundaryConditions(Document doc, Reference reference, bool isConstrainedOnHost, XYZ point)
      Description: Checks if the provided host can host a point boundary conditions.
      @doc: The document containing both the host and the boundary conditions.
      @reference: A reference to an analytical element.
      @isConstrainedOnHost: A value indicating if the point boundary conditions is constrained on host or not.
      @point: Position of the boundary conditions. This is taken into account only if isConstrainedOnHost is false.
      Returns: True if a point boundary conditions can be placed on the input host.
      Throws ArgumentNullException: A non-optional argument was null
    static BoundaryConditions CreateAreaBoundaryConditions(Document doc, ElementId hostElemId, IList<CurveLoop> loops, TranslationRotationValue X_Translation, double X_TranslationSpringModulus, TranslationRotationValue Y_Translation, double Y_TranslationSpringModulus, TranslationRotationValue Z_Translation, double Z_TranslationSpringModulus)
      Description: Creates a new custom area boundary conditions within the project.
      @doc: The document in which to create the boundary conditions.
      @hostElemId: The AnalyticalElement host element for the area boundary conditions.
      @loops: The loops that define geometry of the area boundary conditions. The curve loop collection should contain only closed loops.
      ...
      Returns: If successful, returns the newly created boundary conditions with the BoundaryConditionsType = 2 - "Area". Otherwise it returns .
      ...
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static BoundaryConditions CreateAreaBoundaryConditions(Document doc, Reference reference, TranslationRotationValue X_Translation, double X_TranslationSpringModulus, TranslationRotationValue Y_Translation, double Y_TranslationSpringModulus, TranslationRotationValue Z_Translation, double Z_TranslationSpringModulus)
      Description: Creates a new hosted area boundary conditions within the project.
      @doc: The document in which to create the boundary conditions.
      @reference: A reference to an analytical element face.
      @X_Translation: A value indicating the X axis translation option.
      @X_TranslationSpringModulus: Translation Spring Modulus for X axis. Ignored if X_Translation is not "Spring".
      @Y_Translation: A value indicating the Y axis translation option.
      @Y_TranslationSpringModulus: Translation Spring Modulus for Y axis. Ignored if Y_Translation is not "Spring".
      ...
      @Z_TranslationSpringModulus: Translation Spring Modulus for Z axis. Ignored if Z_Translation is not "Spring".
      Returns: If successful, returns the newly created boundary conditions with the BoundaryConditionsType = 2 - "Area". Otherwise it returns .
      Throws ArgumentException: reference is not a valid host for area boundary conditions. -or- Area boundary conditions already exist on the selected reference.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static BoundaryConditions CreateLineBoundaryConditions(Document doc, ElementId hostElemId, Curve curve, TranslationRotationValue X_Translation, double X_TranslationSpringModulus, TranslationRotationValue Y_Translation, double Y_TranslationSpringModulus, TranslationRotationValue Z_Translation, double Z_TranslationSpringModulus, TranslationRotationValue X_Rotation, double X_RotationSpringModulus)
      Description: Creates a new custom line boundary conditions within the project.
      @doc: The document in which to create the boundary conditions.
      @hostElemId: The AnalyticalElement host element for the line boundary conditions.
      @curve: Curve of the boundary conditions.
      @X_Translation: A value indicating the X axis translation option.
      @X_TranslationSpringModulus: Translation Spring Modulus for X axis. Ignored if X_Translation is not "Spring".
      @Y_Translation: A value indicating the Y axis translation option.
      @Y_TranslationSpringModulus: Translation Spring Modulus for Y axis. Ignored if Y_Translation is not "Spring".
      @Z_Translation: A value indicating the Z axis translation option.
      @Z_TranslationSpringModulus: Translation Spring Modulus for Z axis. Ignored if Z_Translation is not "Spring".
      @X_Rotation: A value indicating the X axis rotation option.
      @X_RotationSpringModulus: Rotation Spring Modulus for X axis. Ignored if X_Rotation is not "Spring".
      Returns: If successful, returns the newly created boundary conditions with the BoundaryConditionsType = 1 - "Line". Otherwise it returns .
      Throws ArgumentException: The provided curve is not supported. -or- The input curve is not bound. -or- hostElemId is not a valid host for line boundary conditions. -or- Line boundary conditions already exist on the selected hostElemId.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static BoundaryConditions CreateLineBoundaryConditions(Document doc, Reference reference, TranslationRotationValue X_Translation, double X_TranslationSpringModulus, TranslationRotationValue Y_Translation, double Y_TranslationSpringModulus, TranslationRotationValue Z_Translation, double Z_TranslationSpringModulus, TranslationRotationValue X_Rotation, double X_RotationSpringModulus)
      Description: Creates a new hosted line boundary conditions within the project.
      @doc: The document in which to create the boundary conditions.
      @reference: A reference to an analytical element edge.
      @X_Translation: A value indicating the X axis translation option.
      @X_TranslationSpringModulus: Translation Spring Modulus for X axis. Ignored if X_Translation is not "Spring".
      @Y_Translation: A value indicating the Y axis translation option.
      @Y_TranslationSpringModulus: Translation Spring Modulus for Y axis. Ignored if Y_Translation is not "Spring".
      ...
      @X_RotationSpringModulus: Rotation Spring Modulus for X axis. Ignored if X_Rotation is not "Spring".
      Returns: If successful, returns the newly created boundary conditions with the BoundaryConditionsType = 1 - "Line". Otherwise it returns .
      Throws ArgumentException: reference is not a valid host for line boundary conditions. -or- Line boundary conditions already exist on the selected reference.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static BoundaryConditions CreatePointBoundaryConditions(Document doc, ElementId hostElemId, XYZ point, TranslationRotationValue X_Translation, double X_TranslationSpringModulus, TranslationRotationValue Y_Translation, double Y_TranslationSpringModulus, TranslationRotationValue Z_Translation, double Z_TranslationSpringModulus, TranslationRotationValue X_Rotation, double X_RotationSpringModulus, TranslationRotationValue Y_Rotation, double Y_RotationSpringModulus, TranslationRotationValue Z_Rotation, double Z_RotationSpringModulus)
      Description: Creates a new custom point boundary conditions within the project.
      @doc: The document in which to create the boundary conditions.
      @hostElemId: The AnalyticalElement host element for the point boundary conditions.
      @point: The position of point boundary conditions, measured in decimal feet.
      @X_Translation: A value indicating the X axis translation option.
      @X_TranslationSpringModulus: Translation Spring Modulus for X axis. Ignored if X_Translation is not "Spring".
      @Y_Translation: A value indicating the Y axis translation option.
      @Y_TranslationSpringModulus: Translation Spring Modulus for Y axis. Ignored if Y_Translation is not "Spring".
      ...
      @X_Rotation: A value indicating the X axis rotation option.
      @X_RotationSpringModulus: Rotation Spring Modulus for X axis. Ignored if X_Rotation is not "Spring".
      @Y_Rotation: A value indicating the Y axis rotation option.
      @Y_RotationSpringModulus: Rotation Spring Modulus for Y axis. Ignored if Y_Rotation is not "Spring".
      @Z_Rotation: A value indicating the Z axis rotation option.
      @Z_RotationSpringModulus: Rotation Spring Modulus for Z axis. Ignored if Z_Rotation is not "Spring".
      Returns: If successful, returns the newly created boundary conditions with the BoundaryConditionsType = 0 - "Point". Otherwise it returns .
      Throws ArgumentException: hostElemId is not a valid host for point boundary conditions. -or- Point boundary conditions already exist on the selected hostElemId.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static BoundaryConditions CreatePointBoundaryConditions(Document doc, Reference reference, TranslationRotationValue X_Translation, double X_TranslationSpringModulus, TranslationRotationValue Y_Translation, double Y_TranslationSpringModulus, TranslationRotationValue Z_Translation, double Z_TranslationSpringModulus, TranslationRotationValue X_Rotation, double X_RotationSpringModulus, TranslationRotationValue Y_Rotation, double Y_RotationSpringModulus, TranslationRotationValue Z_Rotation, double Z_RotationSpringModulus)
      Description: Creates a new hosted point boundary conditions within the project.
      @doc: The document in which to create the boundary conditions.
      @reference: A reference to an analytical element end.
      @X_Translation: A value indicating the X axis translation option.
      @X_TranslationSpringModulus: Translation Spring Modulus for X axis. Ignored if X_Translation is not "Spring".
      @Y_Translation: A value indicating the Y axis translation option.
      @Y_TranslationSpringModulus: Translation Spring Modulus for Y axis. Ignored if Y_Translation is not "Spring".
      ...
      @Z_RotationSpringModulus: Rotation Spring Modulus for Z axis. Ignored if Z_Rotation is not "Spring".
      Returns: If successful, returns the newly created boundary conditions with the BoundaryConditionsType = 0 - "Point". Otherwise it returns .
      Throws ArgumentException: reference is not a valid host for point boundary conditions. -or- Point boundary conditions already exist on the selected reference.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    ...
    Curve GetCurve()
      ...
      Returns: The curve.
      Throws InvalidObjectException: Boundary condition is not a BoundaryConditionsType::Line type.
    ...
    IList<CurveLoop> GetLoops()
      ...
      Throws InvalidObjectException: Boundary condition is not a BoundaryConditionsType::Area type.
    ...
    static bool IsValidHostId(Document doc, ElementId hostId, BoundaryConditionsType type)
      Description: Checks if the provided host id can host a boundary conditions of the provided type.
      @doc: The document containing both the host and the boundary conditions.
      @hostId: The id of the analytical element that is about to host a boundary conditions.
      @type: The type of the boundary conditions that is about to be hosted.
      Returns: True if an area boundary conditions can be placed on the input host id.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    bool IsValidPreset(BoundaryConditionsPreset preset)
      Description: Checks whether the value set for boundary conditions preset is valid.
      @preset: The value set to be verified.
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    static bool IsValidReference(Document doc, Reference reference, BoundaryConditionsType type, bool isConstrainedOnHost)
      Description: Checks if the provided reference can host a boundary conditions.
      @doc: The document containing both the host and the boundary conditions.
      @reference: A reference to an analytical element.
      @type: The type of the boundary conditions that is about to be hosted.
      @isConstrainedOnHost: A value indicating if the boundary conditions is constrained on host or not.
      Returns: True if a boundary conditions can be placed on the input host.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: A value passed for an enumeration argument is not a member of that enumeration
    void RemoveHostConstraint()
      Description: Removes constraint from host for this boundary conditions.
      Throws ModificationForbiddenException: Boundary condition is a BoundaryConditionsType::Area and the host is an Analytical Panel with curved profile.
    void SetCurve(Curve curve)
      Description: Sets the curve that defines geometry of the line boundary conditions.
      Throws ArgumentException: The input curve is not bound.
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidObjectException: Boundary condition is not a BoundaryConditionsType::Line type.
      Throws InvalidOperationException: This BoundaryConditions is not a hosted object. -or- This BoundaryConditions is constrained on host.
    bool SetLoops(IList<CurveLoop> newLoops)
      Description: Sets the curve loops that define geometry of the area boundary conditions.
      @newLoops: Loops that define new geometry of the area boundary conditions. The curve loop collection should contain only closed loops.
      Returns: Returns true if successful, false otherwise.
      Throws ArgumentException: One of the following requirements is not satisfied : - curve loops newLoops are not planar - curve loops newLoops are self-intersecting - curve loops newLoops contains zero length curves
      Throws ArgumentNullException: A non-optional argument was null
      Throws InvalidObjectException: Boundary condition is not a BoundaryConditionsType::Area type.
      Throws InvalidOperationException: This BoundaryConditions is not a hosted object. -or- This BoundaryConditions is constrained on host.
...
[ENUM] BoundaryConditionsPreset
Full Name: Autodesk.Revit.DB.Structure.BoundaryConditionsPreset
Description: Specifies the type of boundary conditions preset.
Inherits: Enum
...
Full Name: Autodesk.Revit.DB.Structure.ComputationDataFor3DPathSet
Description: This class encapsulates data needed to compute a 3D path set.
Implements: IDisposable
  ...
  PROPERTIES:
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
    RebarTerminationOrientation TerminationOrientationAtEnd { get; set; }
      Description: Identifies the orientation of the termination's (e.g. hook, crank) plane at the end of the bar with respect to the orientation of the last curve and the termination plane normal.Only two values are permitted:Value = Right: The termination is on your right as you stand at the end of the bar, with the bar behind you, taking the termination's normal as "up."Value = Left: The termination is on your left as you stand at the end of the bar, with the bar behind you, taking the termination's normal as "up."The default value is Left.
    RebarTerminationOrientation TerminationOrientationAtStart { get; set; }
  ...
  METHODS:
    static ComputationDataFor3DPathSet CreateFromCurves(IList<Curve> curves)
      Description: Creates a new instance of the ComputationDataFor3DPathSet class from the provided curves.
      @curves: The curves that define the 3D path set. The curves must be coplanar and continuous.
      Returns: Returns a new instance of the ComputationDataFor3DPathSet class.
      Throws ArgumentException: curves are not continuous. They do not form a valid curve loop. -or- The curves are not coplanar.
      Throws ArgumentNullException: A non-optional argument was null
    static ComputationDataFor3DPathSet CreateFromRebar(Rebar rebar, int barPositionIndex)
      Description: Creates a new instance of the ComputationDataFor3DPathSet class from the provided Rebar element.
      @rebar: The rebar element from which the data will be obtained.
      @barPositionIndex: The index of the bar. Should be a number between 0 and Rebar.NumberOfBarPositions - 1.
      Returns: Returns a new instance of the ComputationDataFor3DPathSet class.
      Throws ArgumentException: barPositionIndex should be a number between 0 and Rebar.NumberOfBarPositions - 1. -or- rebar should have coplanar curves.
      Throws ArgumentNullException: A non-optional argument was null
    void Dispose()
    IList<Curve> GetCurves()
      Description: Gets the curves that define the 3D path set.
      Returns: The curves that define the 3D path set.
    XYZ GetTerminationPlaneNormal(int end)
      Description: Gets the plane's normal in which the termination (e.g. hook, crank) at end of bar will stay.
      @end: The end of bar. Should be 0 for start or 1 for end.
      Returns: Returns the plane's normal in which the termination (e.g. hook, crank) at end of bar will stay.
      Throws ArgumentOutOfRangeException: Invalid end.
    void SetTerminationPlaneNormal(int end, XYZ terminationPlaneNormal)
      Description: Sets the plane's normal in which the termination (e.g. hook, crank) at end of bar will stay.
      @end: The end of bar. Should be 0 for start or 1 for end.
      @terminationPlaneNormal: The plane's normal in which the termination (e.g. hook, crank) at end of bar will stay. The normal should be perpendicular to the bar direction at the specified end of bar.
      Throws ArgumentException: Normal terminationPlaneNormal for end end isn't a valid normal for bar.
      Throws ArgumentNullException: A non-optional argument was null
...
Full Name: Autodesk.Revit.DB.Structure.ConstraintToRebarEdgeProperties
Description: This class is used to access properties for a constraint to a rebar's edge.
Implements: IDisposable

  CONSTRUCTORS:
    new ConstraintToRebarEdgeProperties(Rebar targetRebar, int targetEgeIndex)
      Description: Constructs a new instance of ConstraintToRebarEdgeProperties.
      @targetRebar: The target Rebar
      @targetEgeIndex: Identifies the index of the target Rebar's edge. The index should be between 0 and centerlines.Count - 1, where centerlines are computed by suppressing hooks, cranks and fillets for the first bar in the target set.
      Throws ArgumentNullException: A non-optional argument was null
      Throws ArgumentOutOfRangeException: targetEgeIndex is not a valid edge index for the rebar. It should be between 0 and centerlines.Count - 1, where centerlines are computed by suppressing hooks, cranks and fillets for the first bar in set.
    new ConstraintToRebarEdgeProperties(TargetRebarEdge targetRebarEdge)
      Description: Constructs a new instance of ConstraintToRebarEdgeProperties.
      Throws ArgumentNullException: A non-optional argument was null

  PROPERTIES:
    double DistanceToTargetRebar { get; set; }
      Description: The distance to the target rebar's edge. The default value is 0.
    bool IsValidObject { get; }
      Description: Specifies whether the .NET object represents a valid Revit entity.
    bool UsePositiveDirectionForZeroOffset { get; set; }
      Description: If true, when offset is zero, the child bar is placed in the default positive direction to avoid the clash, false otherwise. The default value is true.

  METHODS:
    void Dispose()
    TargetRebarEdge GetTargetRebarEdge()
      Description: Gets the target rebar edge.
      Returns: Returns the target rebar edge.
